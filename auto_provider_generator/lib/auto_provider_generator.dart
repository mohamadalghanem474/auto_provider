...
//         // ----------------------------------------
//         // Read dependencies and filter only Bloc/Cubit types
//         // ----------------------------------------
...
//             .where((n) => blocClasses.containsKey(n)) // ✅ keep only real blocs
//             .toList();

//         // Optional developer feedback
//         for (final d in deps) {
//           final name = d.toTypeValue()?.getDisplayString(withNullability: false) ?? '';
//           if (name.isNotEmpty && !blocClasses.containsKey(name)) {
//             print('⚠️ [AutoProviderGenerator] Ignored non-bloc type: $name on $className');
//           }
//         }
...
//       ..writeln('      default:')
//       ..writeln("        return Scaffold(body: Center(child: Text('You Must Add @AutoProvid() On Route \$name', textDirection: TextDirection.ltr),));")
//       ..writeln('    }')
//       ..writeln('  }')
//       ..writeln()
...
//       ..writeln('    return MaterialPageRoute(settings: settings, builder: (_) => find(settings.name));')
//       ..writeln('  }')
//       ..writeln('}');
...
class _AutoProviderBuilder implements Builder {
  ...
  Future<void> build(BuildStep buildStep) async {
    ...
    final depToRoutes = <String, Set<String>>{};
    final screenDeps = <String, List<String>>{};
    final allRoutes = <String>{}; // ✅ collect all route names
      ...
      for (final element in lib.exportNamespace.definedNames2.values) {
        ...
        final depNames = deps
            .map((d) =>
                d.toTypeValue()?.getDisplayString(withNullability: false) ?? '')
            .where((n) => n.isNotEmpty)
            .where((n) => blocClasses.containsKey(n)) // ✅ keep only Bloc/Cubit
            .toList();
        ...
        for (final d in deps) {
          ...
          if (name.isNotEmpty && !blocClasses.containsKey(name)) {
            print(
                '⚠️ [AutoProviderGenerator] Ignored non-bloc type: $name on $className');
          }
        }

        screenDeps[className] = depNames;

        // find routeName static field
        String? routeName;
        for (final field in element.fields) {
          if (field.isStatic && field.name == 'routeName') {
            final val = field.computeConstantValue()?.toStringValue();
        ...
        if (routeName != null) {
          allRoutes.add('$className.routeName'); // ✅ collect for _allRoutes
        }
    ...
    // ----------------------------------------
    // Build sets for each dependency
    // ----------------------------------------
    final depSets = StringBuffer();
    ...
    // ✅ Add a set for ALL routes
    depSets.writeln('  /// All Defined @AutoProvider routes.');
    depSets.writeln('  static final managedRoutes = {');
    ...
    final buffer = StringBuffer()
      ...
      ..writeln('    final name = route?.settings.name;')
      ..writeln('    if (name == null) return;')
      ..writeln('    if (!managedRoutes.contains(name)) return;');
    for (final dep in depToRoutes.keys) {
      final setName = '_${_camel(dep)}';
      buffer
        ..writeln('    if (GetIt.I.checkLazySingletonInstanceExists<$dep>()) {')
    ...
    buffer
      ...
      ..writeln()
      ..writeln('''
  /// If Use defaultOnGenerateRoute Required Add @AutoProvid() On All Route
  /// You Can Customize With PageRouteBuilder In Other Yor onGenerateRoute
  /// You Can Use Mix Of Both With @AutoProvid Or Without On Screen And Use onGenerateRoute
  /// ```dart
  /// class AppRouter {
  ///   static Route onGenerateRoute(RouteSettings settings) {
  ///     if (AutoProvider.managedRoutes.contains(settings.name)) {
  ///       /// Should be managed by AutoProvider
  ///       switch (settings.name) {
  ///         case HomeScreen.routeName:
  ///           /// Should be managed by AutoProvider with transition
  ...
  ///           );
  ///         default:
  ///           /// Should be managed by AutoProvider without transition
  ///           return MaterialPageRoute(
  ///             settings: settings,
  ///             builder: (_) => AutoProvider.find(settings.name),
  ///           );
  ///       }
  ///       /// Should not be managed by AutoProvider With Or Without transition
  ///     } else {
  ///       switch (settings.name) {
  ...
  ///           return MaterialPageRoute(
  ///             settings: settings,
  ///             builder: (_) => Scaffold(body: Center(child: Text('No route defined \${settings.name}'))),
  ///           );
  ///       }
  ///     }
  ///   }
